defmodule Scry.Test.Search.MixProject do
  use Mix.Project
  ...
  defp deps do
    [
      # === SCRY CORE / SCRY SEARCH ===
      # Local path dependencies, not Hex version constraints, since
      # neither is published to Hex yet. Both real, not test-only:
      # this package parses query text through Scry.Search.parse/1 and
      # drives Scry.Search.Executor.run/3 directly.
      {:scry_core, path: "../scry_core"},
      {:scry_search, path: "../scry_search"},
  ...
  defp description do
    "Shared test fixtures for scry_search: one seed dataset servable through Scry.Test." <>
      "Search.Conn's own articles/0 constructor, plus scry_core's own mix scry.query/" <>
      "scry.iex (configured here) for exercising it. Not a cross-backend parity suite -- " <>
      "scry_search has exactly one executor, no interchangeable EngineBehaviour backends " <>
      "to compare."
  end
  ...
  defp package do
    [
      licenses: ["MIT"],
      links: %{"GitHub" => "https://github.com/joetjen/scry_test_search"},
  ...
  defp docs do
    [
      main: "readme",
      source_url: "https://github.com/joetjen/scry_test_search",
